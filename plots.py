...
def plot_adjustment(result, target_name='y', score_name='score', groups=10,
    ...
    ymean = grouped_R['y'].mean()
    smean = grouped_R['score'].mean()
    
    grouped_R['real_lift'] = grouped_R['y'] / ymean
    grouped_R['pred_lift'] = grouped_R['score'] / smean
    
    if lift:
        grouped_R[['pred_lift', 'real_lift']].plot(marker='.')
    else:
        grouped_R[['score', 'y']].plot(marker='.')
